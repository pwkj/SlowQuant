...
def energy_vcc(
    ...
    return expectation_value(
        ...
        wf.thetas,
        wf.vcc_layout,
    ) / np.matmul(wf.ci_coeffs, wf.ci_coeffs)
...
def active_space_parameter_gradient(
    ...
    E = expectation_value_mat(wf.ci_coeffs, Hamiltonian, wf.ci_coeffs) / np.matmul(wf.ci_coeffs, wf.ci_coeffs)
    for i in range(len(theta_params)):  # pylint: disable=consider-using-enumerate
        ...
        E_plus = expectation_value_mat(wf.ci_coeffs, Hamiltonian, wf.ci_coeffs) / np.matmul(wf.ci_coeffs, wf.ci_coeffs) 
        theta_params[i] -= step_size
        wf.thetas = theta_params
