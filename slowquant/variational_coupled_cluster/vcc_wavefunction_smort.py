...
from slowquant.variational_coupled_cluster.operator_matrix import construct_vcc_state 
from slowquant.unitary_coupled_cluster.operators import G1, G2
from slowquant.unitary_coupled_cluster.fermionic_operator import FermionicOperator

class SmortWaveFunctionVCC:
    def __init__(
        self,
        num_spin_orbs: int,
        num_elec: int,
        ...
        self.vcc_layout = VccStructure()
        if "s" in excitations.lower():
            self.vcc_layout.add_singles(self.active_occ_spin_idx_shifted, self.active_unocc_spin_idx_shifted)
        if "d" in excitations.lower():
            self.vcc_layout.add_doubles(self.active_occ_spin_idx_shifted, self.active_unocc_spin_idx_shifted)
        # p for pair doubles lol
        if "p" in excitations.lower():
            self.vcc_layout.add_pair_doubles(self.active_occ_idx_shifted, self.active_unocc_idx_shifted)
        if "t" in excitations.lower():
            self.vcc_layout.add_triples(self.active_occ_spin_idx_shifted, self.active_unocc_spin_idx_shifted)
        if "q" in excitations.lower():
            self.vcc_layout.add_quadruples(
                self.active_occ_spin_idx_shifted, self.active_unocc_spin_idx_shifted
            )
        if "5" in excitations.lower():
            self.vcc_layout.add_quintuples(
                self.active_occ_spin_idx_shifted, self.active_unocc_spin_idx_shifted
            )
        if "6" in excitations.lower():
            self.vcc_layout.add_sextuples(
                self.active_occ_spin_idx_shifted, self.active_unocc_spin_idx_shifted
            )
        self._thetas = np.zeros(self.vcc_layout.n_params).tolist()
        self.vcc_layout.reorder(self.num_spin_orbs)
    ...
    def ci_coeffs(self) -> np.ndarray:
        ...
        if self._ci_coeffs is None:
            self._ci_coeffs = construct_vcc_state(
                self.csf_coeffs,
    ...
    def run_vcc(
        ...
        """Run optimization of VCC wave function.
        ...
        for exc_type in self.vcc_layout.excitation_operator_type:
            if exc_type in ("sa_single", "single"):
                num_theta1 += 1
            elif exc_type in ("sa_double_1", "sa_double_2", "double"):
                num_theta2 += 1
...
def energy_vcc(
    ...
    S = FermionicOperator({"": []}, {"": 1})
    iden = FermionicOperator({"": []}, {"": 1})
    largest_idx = wf.num_spin_orbs - 1
    for theta, exc_idx, exc_type in zip(wf.thetas, wf.vcc_layout.excitation_indicies, wf.vcc_layout.excitation_operator_type):
        if exc_type == "single":
            (i, a) = exc_idx
            T = theta*G1(i, a)
            if a < largest_idx:
                if largest_idx%2 == 0:
                    H = H.get_folded_operator(0, largest_idx//2, 1)
                    S = S.get_folded_operator(0, largest_idx//2, 1)
                print(a)
                largest_idx = a
            if a > largest_idx:
                raise ValueError("Encounted too large index")
        elif exc_type == "double":
            (i,j, a,b) = exc_idx
            T = theta*G2(i,j, a,b)
            if a < largest_idx and b < largest_idx:
                if largest_idx%2 == 0:
                    H = H.get_folded_operator(0, largest_idx//2, 1)
                    S = S.get_folded_operator(0, largest_idx//2, 1)
                largest_idx = max(a, b)
            if a > largest_idx or b > largest_idx:
                raise ValueError("Encounted too large index")
        else:
            raise ValueError(f"Got unknown excitation type: {exc_type}")
        H = (iden + T.dagger) * H * (iden + T)
        S = (iden + T.dagger) * S * (iden + T)
    if len(H.get_folded_operator(wf.num_elec//2, 0, 1).factors) != 1:
        raise ValueError("H has not collapsed")
    if len(S.get_folded_operator(wf.num_elec//2, 0, 1).factors) != 1:
        raise ValueError("S has not collapsed")
    return H.get_folded_operator(wf.num_elec//2, 0, 1).factors[""] / S.get_folded_operator(wf.num_elec//2, 0, 1).factors[""]
...
def active_space_parameter_gradient(
    ...
    E = energy_vcc(parameters, False, wf)
    for i in range(len(theta_params)):  # pylint: disable=consider-using-enumerate
        ...
        E_plus = energy_vcc(parameters, False, wf)
        theta_params[i] -= step_size
        wf.thetas = theta_params
...
def load_wavefunction(filename: str) -> WaveFunctionVCC:
    ...
    energy = energy_vcc(wf.thetas, False, wf)  # pylint: disable=protected-access
